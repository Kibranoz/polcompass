...
export default function App() {
  ...
  useEffect(() => {
    let url = CONFIG.URL + "/polcompass/first";
    if (params.id) {
      url = CONFIG.URL + "/polcompass?id=" + params.id;
    }
    fetch(url, {
      method: "GET",
    })
    ...
    <>
      <div className="App">
        <div className="overviewArea">
          <Grid
            fieldNames={{ x: fieldXName, y: fieldYName }}
            questionNumbers={{
              x: fieldXQuestionNumber,
              y: fieldYQuestionNumber,
            }}
            economic={fieldX}
            social={fieldY}
          ></Grid>
          <Question
            questionNumber={fieldXQuestionNumber + fieldYQuestionNumber}
            questionString={questions[questionIndex]["question"]}
            index={questionIndex}
          ></Question>
        </div>
